...
├── requirements.txt      # capability-complete: LangChain + ChromaDB + LangGraph + MCP + FastEmbed
├── .python-version       # 3.12 — every wheel above is prebuilt for cp312 on Streamlit Cloud
├── runtime.txt           # python-3.12 (belt-and-suspenders for the Cloud build)
├── requirements-min.txt  # fallback if the full build ever OOMs (Labs 1–2 + graded UI)
└── content/
    ├── _data/                   sample PDFs
