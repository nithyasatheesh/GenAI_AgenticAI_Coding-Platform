...
**Scoring** — each lab is out of **100**, weighted **T1 10 · T2 25 · T3 25 ·
T4 15 · T5 25** (the per-check rubric points are auto-scaled to the task weight).
...
├── requirements.txt      # DEFAULT — pure-wheel deps, builds on Streamlit Cloud with no compiler
├── requirements-full.txt # + ChromaDB, FastEmbed (keyless embeddings), MCP lab — local / Docker
└── content/
    ├── _data/                   sample PDFs
...
| Vector store | `requirements-full.txt` → **real ChromaDB** (`chromadb.EphemeralClient`). Default deploy → `langchain-core` **`InMemoryVectorStore`** (still a real vector store — real embeddings, real cosine retrieval). `make_vectorstore()` picks whichever is importable. |
| Embeddings | **OpenAI mode → real `OpenAIEmbeddings`** (works on the default deploy). Keyless: `requirements-full.txt` → real local **FastEmbed** ONNX model; without it, a deterministic fallback (weak retrieval — use OpenAI mode for real keyless-free embeddings). |
| MCP | `requirements-full.txt`. `mcp_server.py` (FastMCP) is launched as a **real subprocess**; `langchain-mcp-adapters` speaks MCP over stdio. Without those packages the MCP lab's Run is blocked with a clear message. |
...
pip install -r requirements.txt          # or requirements-full.txt for ChromaDB + FastEmbed + MCP
streamlit run streamlit_app.py
```

To use real LLMs, set `OPENAI_API_KEY` (or `ANTHROPIC_API_KEY`) first. With
`requirements-full.txt`, the first Run of Lab 1 downloads the ~90 MB FastEmbed
model once.
...
3. First build uses **`requirements.txt`** (the default) — **pure-wheel deps, no
   compiler, ~1 min.** Verified: 91-package resolve, exit 0, no
   `onnxruntime`/`chroma-hnswlib`/`grpcio`.
   *Do **not** point Cloud at `requirements-full.txt` — chromadb / fastembed
   won't build there.*
4. **Settings → Secrets** → add `OPENAI_API_KEY` (or `ANTHROPIC_API_KEY`) to turn
   on real LLM mode + real OpenAI embeddings + Labs 2–4.

| Deploy log says | Fix |
|---|---|
| `installer returned a non-zero exit code` / build fails on `onnxruntime`, `chroma-hnswlib`, `grpcio`, a rust crate | you're installing `requirements-full.txt` — switch the app to `requirements.txt` (the default) and Reboot. |
| `ModuleNotFoundError: chromadb` / `mcp` at runtime | expected on the default deploy — Lab 1 uses the in-memory store; the MCP lab is blocked with a message. For those, deploy `requirements-full.txt` in Docker/VM. |
