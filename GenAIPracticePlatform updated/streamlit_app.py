...
PIP_NAME = {
    ...
    "langgraph": "langgraph", "chromadb": "chromadb", "pypdf": "pypdf",
    "langchain": "langchain", "dotenv": "python-dotenv",
}
...
def dependency_report(requires: list[str], mode: str) -> list[str]:
    ...
    if mode == "real" and "langchain_openai" not in need:
        need.append("langchain_openai")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        pick = st.radio("LLM mode", ["🧪 Mock — no API key", "🔌 Real LLM — API key"],
                        help="Mock: local fake model + embeddings, offline. "
                             "Real: OpenAI via langchain-openai, needs OPENAI_API_KEY.")
        mode = "real" if pick.startswith("🔌") else "mock"
        if mode == "mock":
            st.caption("✅ Offline. The LangChain **pipeline is real**; only the model "
                       "and embeddings are local stand-ins.")
        elif os.environ.get("OPENAI_API_KEY", "").startswith("sk-"):
            st.caption("✅ `OPENAI_API_KEY` detected — real OpenAI calls will be made.")
        else:
            st.caption("⚠ No `OPENAI_API_KEY`. Set it in **App → Settings → Secrets** "
                       "(cloud) or your environment (local), or use Mock mode.")
        ...
        if do_run:
            ...
            if missing:
                st.error(
                    ...
                    + "\n\nThey are listed in `requirements.txt` already — the "
                    "environment needs a rebuild (cloud) or "
                    "`pip install -r requirements.txt` (local). Nothing is mocked or "
                    "skipped.")
            elif mode == "real" and not os.environ.get("OPENAI_API_KEY", "").startswith("sk-"):
                st.error("**Real LLM mode needs `OPENAI_API_KEY`.** Set it in "
                         "*App → Settings → Secrets* (cloud) or your shell (local), "
                         "or switch to Mock mode in the sidebar.")
            else:
