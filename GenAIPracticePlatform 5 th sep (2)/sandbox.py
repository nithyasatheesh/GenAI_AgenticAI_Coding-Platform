...
def load_sample_docs():
    """Load the bundled PDF as LangChain `Document`s. Uses `PyPDFLoader` if
    `langchain-community` is installed, else reads it directly with `pypdf`
    (so `langchain-community` is optional for the minimal deploy)."""
    ...
    except Exception:  # noqa: BLE001 — langchain-community not installed
        from pypdf import PdfReader
        reader = PdfReader(SAMPLE_PDF)
        return [Document(page_content=(pg.extract_text() or ""),
                         metadata={"source": SAMPLE_PDF, "page": i})
